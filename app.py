...
def test(testid):
	...
	if request.method == 'POST':
		cur = mysql.connection.cursor()
		results1 = cur.execute('SELECT qid from questions where test_id = %s',[testid])
		results1 = cur.fetchall()
		completed=1

		for sa in results1:
			answerByStudent = request.form[str(sa[0])]
...
def totmarks(username,tests): 
	...
	for test in tests:
		...
		if str(results[0]) == 'None':
			results[0] = 0
		test['marks'] = results[0]
		if "Decimal" not in str(results[0]): 
			mstr = str(results[0]).replace('Decimal', '')
			results[0] = mstr
			test['marks'] = results[0]
	return tests
